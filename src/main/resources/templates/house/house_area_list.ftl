...
<html>
    ...
    <body>
        ...
        <fieldset class="layui-elem-field">
             <legend>查询条件</legend>
            <form action="${request.contextPath}/backpage/houseArea/list" id="formId" style="padding-left:33px;">
                 <ul class="findTool">
        ...
        $('#toAdd').on('click', function() {
            window.location.href = "${request.contextPath}/backpage/houseArea/toinput"
        });
        //跳转修改
        function toEdit(id){
            window.location.href = "${request.contextPath}/backpage/houseArea/toinput?id="+id;
        }
        //删除
        ...
        function delById(id){
            if(confirm("删除了不能恢复,确定要删除？")){
                var url = '${request.contextPath}/backpage/houseArea/del';
                $.post(url, {
                    'id':id
                }, function(data) {
                     if("0000"==data.code){
                        layer.msg("删除成功");
                        window.location.href = "${request.contextPath}/backpage/houseArea/list";
                    }else{
                        layer.msg("删除失败");
                    }
                });
